...
# Safe to share across sessions: read-only, stateless inference
@st.cache_resource
...
class GestureProcessor(VideoProcessorBase):
    def __init__(self):
        # Each connected user gets their OWN instance of everything below —
        # no more sharing a single landmarker/buffer across all visitors
    ...
    def recv(self, frame):
        ...
        dynamic_label = ""
        if len(self.frame_buffer) == SEQUENCE_LENGTH and self.cooldown == 0:
            ...
            if predicted_label != "None" and confidence >= CONFIDENCE_THRESHOLD:
                ...
                self.last_dynamic_label = dynamic_label
                self.cooldown = COOLDOWN_FRAMES

        if self.cooldown > 0:
            self.cooldown -= 1
            dynamic_label = self.last_dynamic_label
